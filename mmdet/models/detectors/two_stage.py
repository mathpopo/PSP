...
class TwoStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        if train_cfg is not None and 'mll' in train_cfg:
            self.conv = nn.Conv2d(2048, 2048, kernel_size=3, stride=1, padding=1)
            self.gap = nn.AdaptiveAvgPool2d((1, 1))
            self.gmp = nn.AdaptiveMaxPool2d((1, 1))
            self.fc = nn.Linear(2048, roi_head.bbox_head.num_classes)
    ...
    def forward_train(self,
        ...
        if self.train_cfg is not None and 'mll' in self.train_cfg:
            ...
            if outs.is_cuda:
                ml_gt = ml_gt.cuda()
            gt_labels_ = [gt_labels[i].unique() for i in range(len(gt_labels))]
